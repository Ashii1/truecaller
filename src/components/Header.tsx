      ...
      <div className="mx-auto flex min-h-16 max-w-4xl items-center justify-between gap-2 px-3 sm:px-6">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="text-base font-bold text-white">{t('app_title')}</div>
            <div className="truncate text-[11px] text-slate-500">{isDefaultDialer ? t('default_phone_app') : t('phone_setup_required')}</div>
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <LanguageSwitcher variant="header" />
          {onRequestDefaultDialer && !isDefaultDialer && <button onClick={onRequestDefaultDialer} className="hidden items-center gap-2 rounded-xl bg-blue-600 px-3 py-2 text-xs font-bold text-white hover:bg-blue-500 sm:flex"><PhoneCall className="h-3.5 w-3.5" /> {t('set_as_phone_app')}</button>}
          {isDefaultDialer && onToggleShield && <button onClick={onToggleShield} className={`rounded-xl px-3 py-2 text-xs font-bold ${settings?.masterEnabled !== false ? 'bg-blue-600 text-white' : 'bg-rose-600 text-white'}`}>{settings?.masterEnabled !== false ? t('protected') : t('paused')}</button>}
          <button onClick={() => setShowSettings(true)} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-700 bg-slate-900 text-slate-300 transition hover:bg-slate-800 hover:text-white" aria-label={t('settings_title')}><Settings className="h-4 w-4" /></button>
        </div>
      </div>
    </header>
    ...
    {showSettings && <div className="fixed inset-0 z-[70] bg-black/60 p-0 sm:p-4" onMouseDown={() => setShowSettings(false)}>
      <section className="absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto border-l border-slate-700 bg-slate-950 shadow-2xl sm:relative sm:mx-auto sm:my-8 sm:h-auto sm:max-h-[calc(100vh-4rem)] sm:rounded-3xl sm:border" onMouseDown={e => e.stopPropagation()}>
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-800 bg-slate-950/95 p-5 backdrop-blur">
          <div><h2 className="text-lg font-bold text-white">{t('settings_title')}</h2><p className="mt-0.5 text-xs text-slate-400">{t('settings_subtitle')}</p></div>
          <button onClick={() => setShowSettings(false)} className="rounded-full p-2 text-slate-400 hover:bg-slate-800 hover:text-white" aria-label={t('close')}><X className="h-5 w-5" /></button>
        </div>
        <div className="space-y-3 p-4">
          ...
          <button onClick={() => { setShowSettings(false); setShowTheme(true); }} className="flex w-full items-center gap-3 rounded-2xl border border-blue-500/20 bg-blue-500/5 p-4 text-left hover:bg-blue-500/10">
            ...
            <span className="min-w-0 flex-1"><span className="block text-sm font-bold text-white">{t('appearance_title')}</span><span className="mt-1 block text-xs leading-5 text-slate-400">{t('appearance_desc')}</span></span>
            <span className="text-xs font-bold text-blue-400">{t('customize')}</span>
          </button>

          {!isDefaultDialer && onRequestDefaultDialer && <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4"><div className="flex items-start gap-3"><PhoneCall className="mt-0.5 h-5 w-5 shrink-0 text-amber-400" /><div className="min-w-0 flex-1"><div className="text-sm font-bold text-white">{t('default_phone_title')}</div><div className="mt-1 text-xs leading-5 text-slate-400">{t('default_phone_desc')}</div><button onClick={onRequestDefaultDialer} className="mt-3 rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white hover:bg-blue-500">{t('set_as_default_phone_button')}</button></div></div></div>}

          <SettingRow icon={<LockKeyhole className="h-4 w-4" />} title={t('private_notification_title')} description={t('private_notification_desc')} checked={privacy.privacyMode} onChange={v => updatePrivacy('privacyMode', v)} />
          <SettingRow icon={<Bell className="h-4 w-4" />} title={t('detailed_notifications_title')} description={t('detailed_notifications_desc')} checked={privacy.showCallerDetailsInNotifications} onChange={v => updatePrivacy('showCallerDetailsInNotifications', v)} />
          <SettingRow icon={<MicOff className="h-4 w-4" />} title={t('call_recording_title')} description={t('call_recording_desc')} checked={privacy.callRecordingEnabled} onChange={v => updatePrivacy('callRecordingEnabled', v)} danger={privacy.callRecordingEnabled} />
          <SettingRow icon={<Settings className="h-4 w-4" />} title={t('clipboard_paste_title')} description={t('clipboard_paste_desc')} checked={privacy.clipboardPasteDetection} onChange={v => updatePrivacy('clipboardPasteDetection', v)} />

          {onToggleAutoCancel && <SettingRow icon={<ShieldCheck className="h-4 w-4" />} title={t('autocancel_spam_title')} description={t('autocancel_spam_desc')} checked={autoCancelEnabled !== false} onChange={() => onToggleAutoCancel()} />}

          <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-4">
            <div className="flex items-start gap-3"><Siren className="mt-0.5 h-5 w-5 shrink-0 text-amber-400" /><div className="min-w-0"><div className="text-sm font-bold text-white">{t('emergency_repeat_title')}</div><div className="mt-1 text-xs leading-5 text-slate-400">{t('emergency_repeat_desc')}</div></div></div>
            <div className="mt-3"><SettingRow icon={<Siren className="h-4 w-4" />} title={t('enable_repeat_alert')} description={t('repeat_alert_desc')} checked={privacy.emergencyRepeatEnabled} onChange={v => updatePrivacy('emergencyRepeatEnabled', v)} /></div>
            <div className="mt-3 grid grid-cols-2 gap-2">
              <label className="rounded-xl border border-slate-700 bg-slate-900 p-3 text-xs text-slate-400">{t('calls_needed')}<select value={privacy.emergencyRepeatThreshold} onChange={e => updatePrivacy('emergencyRepeatThreshold', Number(e.target.value) as 3|4|5)} className="mt-1 w-full rounded-lg bg-slate-800 px-2 py-2 text-sm font-semibold text-white outline-none"><option value={3}>{t('calls_count_3')}</option><option value={4}>{t('calls_count_4')}</option><option value={5}>{t('calls_count_5')}</option></select></label>
              <label className="rounded-xl border border-slate-700 bg-slate-900 p-3 text-xs text-slate-400">{t('time_window')}<select value={privacy.emergencyRepeatWindow} onChange={e => updatePrivacy('emergencyRepeatWindow', Number(e.target.value) as 3|5|10)} className="mt-1 w-full rounded-lg bg-slate-800 px-2 py-2 text-sm font-semibold text-white outline-none"><option value={3}>{t('time_window_3')}</option><option value={5}>{t('time_window_5')}</option><option value={10}>{t('time_window_10')}</option></select></label>
            </div>
            <div className="mt-3 rounded-xl bg-slate-900/70 p-3 text-[11px] leading-5 text-slate-500">{t('emergency_footer_note')}</div>
          </div>

          {onRequestDefaultDialer && <button onClick={onRequestDefaultDialer} className="w-full rounded-2xl border border-slate-700 bg-slate-900 px-4 py-3 text-left text-sm font-semibold text-slate-200 hover:bg-slate-800">{isDefaultDialer ? t('default_phone_status_active') : t('set_as_default_phone_button')}<span className="mt-1 block text-xs font-normal text-slate-500">{t('default_phone_role_desc')}</span></button>}
          {onOpenPermissionCenter && <button onClick={() => { setShowSettings(false); onOpenPermissionCenter(); }} className="w-full rounded-2xl border border-slate-700 bg-slate-900 px-4 py-3 text-left text-sm font-semibold text-slate-200 hover:bg-slate-800">{t('permission_center')}<span className="mt-1 block text-xs font-normal text-slate-500">{t('permission_center_desc')}</span></button>}
          {onSyncDatabase && <button onClick={onSyncDatabase} className="flex w-full items-center gap-3 rounded-2xl border border-slate-700 bg-slate-900 p-4 text-left"><Database className="h-5 w-5 text-cyan-400" /><span className="flex-1"><b className="block text-sm text-white">{t('sync_device_data')}</b><span className="mt-1 block text-xs text-slate-500">{isSyncing ? t('synchronizing') : t('sync_device_desc')}</span></span></button>}
          {onOpenDataSources && <button onClick={() => { setShowSettings(false); onOpenDataSources(); }} className="flex w-full items-center gap-3 rounded-2xl border border-slate-700 bg-slate-900 p-4 text-left"><Database className="h-5 w-5 text-indigo-400" /><span><b className="block text-sm text-white">{t('data_sources_privacy')}</b><span className="mt-1 block text-xs text-slate-500">{t('data_sources_desc')}</span></span></button>}
          {onOpenDiagnostics && <button onClick={() => { setShowSettings(false); onOpenDiagnostics(); }} className="flex w-full items-center gap-3 rounded-2xl border border-slate-700 bg-slate-900 p-4 text-left"><Stethoscope className="h-5 w-5 text-amber-400" /><span><b className="block text-sm text-white">{t('system_diagnostics')}</b><span className="mt-1 block text-xs text-slate-500">{t('system_diagnostics_desc')}</span></span></button>}
          {onOpenInstallModal && <button onClick={() => { setShowSettings(false); onOpenInstallModal(); }} className="flex w-full items-center gap-3 rounded-2xl border border-slate-700 bg-slate-900 p-4 text-left"><Download className="h-5 w-5 text-emerald-400" /><span><b className="block text-sm text-white">{t('install_packaging')}</b><span className="mt-1 block text-xs text-slate-500">{t('install_packaging_desc')}</span></span></button>}

          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 text-xs leading-5 text-slate-500">{t('security_principle_note')}</div>
        </div>
      </section>
    </div>}
