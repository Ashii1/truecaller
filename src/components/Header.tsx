      ...
      {showSettings && <div className="fixed inset-0 z-[70] bg-black/60 p-0 sm:p-4" onMouseDown={() => setShowSettings(false)}>
        <section className="absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto border-l border-slate-700 bg-slate-950 shadow-2xl sm:relative sm:mx-auto sm:my-8 sm:h-auto sm:max-h-[calc(100vh-4rem)] sm:rounded-3xl sm:border" onMouseDown={e => e.stopPropagation()}>
          ...
          <div className="space-y-3 p-4">
            <button onClick={() => { setShowSettings(false); setShowTheme(true); }} className="flex w-full items-center gap-3 rounded-2xl border border-blue-500/20 bg-blue-500/5 p-4 text-left hover:bg-blue-500/10"><span className="grid h-10 w-10 place-items-center rounded-xl bg-blue-500/10 text-blue-400"><Palette className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-bold text-white">Appearance & colours</span><span className="mt-1 block text-xs leading-5 text-slate-400">Dark / light / system, accent colour, custom colour, density, corners, backgrounds, text size and accessibility.</span></span><span className="text-xs font-bold text-blue-400">Customize</span></button>
