...
import { Bell, CheckCircle2, Clock, PhoneCall, ShieldAlert, ShieldCheck, Users, X, Moon, Gauge, CalendarClock, UserRoundCheck, VolumeX } from 'lucide-react';
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-4 sm:p-5">
          ...
          <section className="space-y-3">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
              <div className="flex gap-2"><input value={trustedInput} onChange={e=>setTrustedInput(e.target.value)} placeholder="Phone number" inputMode="tel" className="min-w-0 flex-1 rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white outline-none focus:border-blue-500" /><button onClick={addTrusted} className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-bold text-white">Add trusted</button></div>
              <div className="mt-3 flex flex-wrap gap-2">{trustedNumbers.length===0 ? <span className="text-xs text-slate-500">No extra trusted numbers.</span> : trustedNumbers.map(n=><button key={n} onClick={()=>setTrustedNumbers(p=>p.filter(x=>x!==n))} className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-[11px] text-emerald-300">{n} ×</button>)}</div>
            </div>
          </section>

          <section className="space-y-3">
            <div className="flex items-center gap-2"><Moon className="h-4 w-4 text-indigo-400" /><div><h3 className="text-sm font-bold text-white">Smart Silent Mode</h3><p className="text-xs text-slate-500">Keep important calls audible while reducing interruptions.</p></div></div>
            <ChoiceRow value={silentMode} onChange={setSilentMode} options={[['OFF','Off'],['UNKNOWN_ONLY','Unknown callers'],['UNKNOWN_AND_SPAM','Unknown + spam']]} />
          </section>

          <section className="space-y-3">
            <div className="flex items-center gap-2"><VolumeX className="h-4 w-4 text-amber-400" /><div><h3 className="text-sm font-bold text-white">Smart Spam Quieting</h3><p className="text-xs text-slate-500">Choose whether suspicious calls warn, silence, or reject at high confidence.</p></div></div>
            <ChoiceRow value={spamQuietMode} onChange={setSpamQuietMode} options={[['WARN','Warn only'],['SILENCE','Silence spam'],['REJECT_HIGH_RISK','Reject high risk']]} />
          </section>

          <section className="space-y-3">
            <div className="flex items-center gap-2"><Gauge className="h-4 w-4 text-blue-400" /><div><h3 className="text-sm font-bold text-white">Call Priority & Risk</h3><p className="text-xs text-slate-500">Every caller is ranked using the existing Safe / Unknown / Suspicious / High Risk model.</p></div></div>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{[['SAFE','Safe'],['UNKNOWN','Unknown'],['SUSPICIOUS','Suspicious'],['HIGH_RISK','High risk']].map(([id,label])=><div key={id} className="rounded-xl border border-slate-800 bg-slate-900 p-3"><div className="text-[11px] font-bold text-white">{label}</div><div className="mt-1 text-[10px] text-slate-500">{id==='HIGH_RISK'?'Urgent attention':id==='SUSPICIOUS'?'Review carefully':id==='UNKNOWN'?'Needs context':'Normal priority'}</div></div>)}</div>
            <div className="rounded-xl border border-slate-800 bg-slate-900/70 px-3 py-2 text-xs text-slate-400">Current sensitivity: <span className="font-bold text-slate-200">{settings.sensitivity}</span> · {riskDescription}</div>
          </section>

          <section className="space-y-3">
            <div className="flex items-center gap-2"><CalendarClock className="h-4 w-4 text-cyan-400" /><div><h3 className="text-sm font-bold text-white">Callback Reminders</h3><p className="text-xs text-slate-500">Keep a local follow-up queue after missed or unfinished calls.</p></div></div>
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3"><input value={reminderNumber} onChange={e=>setReminderNumber(e.target.value)} placeholder="Number" inputMode="tel" className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-white outline-none" /><input value={reminderLabel} onChange={e=>setReminderLabel(e.target.value)} placeholder="Reason" className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-white outline-none" /><input value={reminderDue} onChange={e=>setReminderDue(e.target.value)} type="datetime-local" className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-white outline-none" /></div>
            <button onClick={addReminder} className="rounded-xl bg-cyan-700 px-3 py-2 text-xs font-bold text-white hover:bg-cyan-600">Add callback reminder</button>
            <div className="space-y-2">{reminders.length===0 ? <div className="text-xs text-slate-500">No callback reminders.</div> : reminders.slice(0,5).map(r=><div key={r.id} className="flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-900 p-3"><div className="min-w-0 flex-1"><div className="text-xs font-bold text-white">{r.label}</div><div className="text-[11px] text-slate-500">{r.number} · {new Date(r.dueAt).toLocaleString()}</div></div><button onClick={()=>setReminders(p=>p.filter(x=>x.id!==r.id))} className="text-[11px] font-bold text-slate-400 hover:text-white">Done</button></div>)}</div>
          </section>

          <section className="space-y-3">
            <div className="flex items-center gap-2"><PhoneCall className="h-4 w-4 text-slate-300" /><div><h3 className="text-sm font-bold text-white">Unknown Caller Handling</h3><p className="text-xs text-slate-500">A separate choice from spam protection, so normal unknown calls are not automatically treated as scams.</p></div></div>
            <ChoiceRow value={unknownMode} onChange={setUnknownMode} options={[['ALLOW','Allow'],['SILENCE','Silence'],['REJECT','Reject']]} />
          </section>
...
function ChoiceRow<T extends string>({ value, onChange, options }: { value:T; onChange:(v:T)=>void; options:[T,string][] }) {
