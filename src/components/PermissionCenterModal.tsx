...
import { ArrowLeft, Bell, CheckCircle2, Clock, PhoneCall, ShieldAlert, ShieldCheck, Users, Moon, Gauge, CalendarClock, UserRoundCheck, VolumeX, Landmark, AlertTriangle, Fingerprint } from 'lucide-react';
      ...
      <section className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-slate-700 bg-slate-950 shadow-2xl">
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-4 sm:p-5">
          <section className="space-y-3">
            ...
            <StatusRow icon={<PhoneCall className="h-4 w-4" />} title="Default Phone App" description="Routes real cellular calls through CallShield InCallService." status={isDefaultDialer ? 'ON' : 'OFF'} action={!isDefaultDialer ? <button onClick={onRequestDefaultDialer} className="rounded-xl bg-blue-600 px-3 py-2 text-xs font-bold text-white hover:bg-blue-500">Set as default</button> : undefined} />
          ...
          <section className="space-y-3"><div className="flex items-center gap-2"><Landmark className="h-4 w-4 text-red-400" /><div><h3 className="text-sm font-bold text-white">Scam & Spoof Protection</h3><p className="text-xs text-slate-500">Local-only heuristics flag financial scam patterns, urgency and impersonation signals. Caller ID is never treated as proof of identity.</p></div></div>
            ...
            <div className="flex items-start gap-2 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-3"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-400" /><p className="text-[11px] leading-5 text-slate-400">CallShield does not claim access to carrier or government fraud databases. These warnings are defensive local signals and can be wrong; verify important callers through a trusted channel.</p></div>
          </section>
