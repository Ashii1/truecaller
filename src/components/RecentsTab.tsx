...
export default function RecentsTab({ calls, lookupProfile, onInitiateCall, onSelectCall, onBlockNumber, onDeleteCall, onClearAllCalls }: RecentsTabProps) {
  ...
  // Call history can be persisted before the contact lookup finishes. Resolve every
  // visible number here as well, so returning to Recents never leaves stale numbers.
  useEffect(() => {
    let cancelled = false;
    if (!telecomBridge.isAndroidEnvironment()) return;
    const numbers = [...new Set(calls.map(call => call.number).filter(Boolean))];
    if (!numbers.length) return;
    Promise.all(numbers.map(async number => [number, telecomBridge.lookupContactName(number)] as const))
      .then(entries => { if (!cancelled) setDeviceNames(previous => ({ ...previous, ...Object.fromEntries(entries.filter(([, name]) => Boolean(name))) })); });
  ...
  const filtered = useMemo(() => {
    ...
    return [...calls].sort((a, b) => b.timestamp - a.timestamp).filter(call => {
      ...
      if (!q) return true;
      const name = (deviceNames[call.number] || call.callerName || '').toLowerCase();
      const number = call.number.replace(/\D/g, '');
      return name.includes(q) || (digits.length > 0 && number.includes(digits));
    });
  }, [calls, query, filter, showMissedOnly, deviceNames]);
