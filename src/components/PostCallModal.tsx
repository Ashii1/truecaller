...
export default function PostCallModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 animate-in zoom-in-95">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-2xl space-y-4 text-white">
        {/* Header */}
        ...
        <div className="p-4 rounded-2xl bg-slate-800/80 border border-slate-700 flex items-center justify-between">
          <div>
            <div className="text-base font-extrabold text-white">
              {postCall.name || 'Unidentified Caller'}
            </div>
        ...
        {savedSuccessMessage && (
          <div className="p-3 rounded-xl bg-emerald-500/20 border border-emerald-500/40 text-xs font-semibold text-emerald-300 flex items-center space-x-2">
            <Check className="w-4 h-4 text-emerald-400" />
            <span>{savedSuccessMessage}</span>
          </div>
        )}

        {/* Spam Report Form Dropdown */}
        {showSpamReport ? (
          <form onSubmit={handleReport} className="p-4 rounded-2xl bg-slate-850 border border-rose-500/30 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => setShowSpamReport(false)}
            ...
            <div className="grid grid-cols-2 gap-1.5 text-xs">
              {(['TELEMARKETING', 'ROBOCALL', 'SCAM', 'DEBT_COLLECTOR', 'CUSTOM'] as SpamCategory[]).map((cat) => (
                <button
                  ...
                  className={`p-2 rounded-xl border text-left font-semibold ${
                    spamCategory === cat
          ...
          <form onSubmit={handleSaveCallNote} className="p-4 rounded-2xl bg-slate-850 border border-slate-700 space-y-3">
            ...
            <textarea
              ...
              placeholder="e.g. Discussed proposal delivery; agreed on Friday follow-up"
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <button
              onClick={() => {
                onAddContact(postCall.number, postCall.name);
                onDismiss();
              }}
              className="p-3 rounded-2xl bg-slate-800 hover:bg-slate-750 border border-slate-700 flex items-center space-x-2.5 font-semibold text-slate-200 transition"
            >
            ...
            <button
              onClick={handleBlock}
              className="p-3 rounded-2xl bg-slate-800 hover:bg-slate-750 border border-slate-700 flex items-center space-x-2.5 font-semibold text-rose-300 transition"
            >
            ...
            {/* Report Spam */}
            <button
              onClick={() => setShowSpamReport(true)}
              className="p-3 rounded-2xl bg-slate-800 hover:bg-slate-750 border border-slate-700 flex items-center space-x-2.5 font-semibold text-amber-300 transition"
            >
        ...
        <div className="pt-2">
          <button
            onClick={onDismiss}
            className="w-full py-2.5 rounded-xl bg-slate-800 hover:bg-slate-750 text-white font-bold text-xs transition"
          >
            Done
          </button>
        </div>
