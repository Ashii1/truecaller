...
export interface IncomingCallState { active:boolean; number:string; callerName:string; carrier:string; location:string; lineType?:string; topTags?:string[]; isSpam:boolean; isVerifiedBusiness?:boolean; spamCategory?:SpamCategory; spamReason?:string; riskScore:number; spamScore?:number; reportsCount:number; countdown:number; status:'RINGING'|'CANCELLED'|'ANSWERED'|'DECLINED'; }
