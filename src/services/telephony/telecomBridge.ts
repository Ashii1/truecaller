...
class TelecomBridgeService {
  ...
  private initEvents(){if(typeof window==='undefined')return;window.__onAndroidTelecomEvent=(type,payload)=>{if(type==='ROLE_STATUS_CHANGED')this.defaultDialerConfirmed=!!payload?.isDefaultDialer;this.listeners.forEach(l=>{try{l(type,payload);}catch(e){console.error(e);}});};window.__onAndroidDialIntent=n=>this.notifyDialIntent(n);}
