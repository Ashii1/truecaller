...
class VigilShieldInCallService : InCallService() {
    ...
    override fun onCallAdded(call: Call) {
            ...
            override fun onStateChanged(call: Call,state:Int){
                ...
                when(state){
                    ...
                    Call.STATE_ACTIVE,Call.STATE_DIALING,Call.STATE_CONNECTING,Call.STATE_HOLDING->{
                        ...
                        // FIX: Don't clear incoming call notification yet - keep tracking it with updated status
                        // FIX: Pass isIncoming flag to prevent redirect to phone app
                        CallNotificationHelper.showOngoingCall(applicationContext,callId,name,number,stateName,details.connectTimeMillis, null, details.isIncoming)
                        // FIX: Always launch in-call UI to keep call in this app
                        launchCallShieldInCallSurface(callId,name,number)
                    }
                    Call.STATE_DISCONNECTED->{
                        CallRingerHelper.stopRinging(applicationContext)
                        // FIX: Only clear notification when truly disconnected
        ...
        if(call.state==Call.STATE_RINGING)handleIncomingCall(call,callId) else if(call.state==Call.STATE_ACTIVE||call.state==Call.STATE_HOLDING){
            // FIX: Pass isIncoming flag to prevent redirect to phone app
            CallNotificationHelper.showOngoingCall(applicationContext,callId,name,number,stateToString(call.state),initial.connectTimeMillis, null, initial.isIncoming)
            // FIX: Ensure in-call UI is visible
            launchCallShieldInCallSurface(callId,name,number)
        }
