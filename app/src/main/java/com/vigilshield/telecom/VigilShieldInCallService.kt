...
/**
 * Android Telecom's single source of truth for real cellular calls.
 * CallShield must own ROLE_DIALER before calls are placed through TelecomManager.
 */
...
class VigilShieldInCallService : InCallService() {
    ...
    override fun onCallAdded(call: Call) {
        ...
        val callId=getCallIdentifier(call); activeCalls[callId]=call
        val callback=object:Call.Callback(){
            override fun onStateChanged(call: Call,state:Int){
                super.onStateChanged(call,state)
                val stateName=stateToString(state); val details=extractCallDetails(call,callId); val name=details.callerDisplayName.orEmpty().ifBlank{details.number.ifBlank{"Unknown caller"}}; val number=details.number
                when(state){
                    ...
                    Call.STATE_ACTIVE,Call.STATE_DIALING,Call.STATE_CONNECTING,Call.STATE_HOLDING -> {
                        ...
                        // Only foreground the UI once the real Telecom call is active/held.
                        // DIALING/CONNECTING can transition rapidly and repeatedly; launching
                        // MainActivity from those callbacks caused OEM Phone handoff/crash races.
                        if(state==Call.STATE_ACTIVE || state==Call.STATE_HOLDING) launchCallShieldInCallSurface(callId,name,number,details.isIncoming)
                    }
                    Call.STATE_DISCONNECTED -> {
                        CallRingerHelper.stopRinging(applicationContext); CallNotificationHelper.clearCall(applicationContext,callId)
                        if(details.isIncoming&&details.connectTimeMillis<=0L) CallNotificationHelper.showMissedCall(applicationContext,name,number)
                    }
                }
    ...
    private fun launchCallShieldInCallSurface(id:String,name:String,number:String,isIncoming:Boolean){runCatching{val intent=Intent(applicationContext,MainActivity::class.java).apply{action="com.vigilshield.telecom.OPEN_OUTGOING_CALL";putExtra("open_call_id",id);putExtra("open_call_number",number);putExtra("open_call_name",name);putExtra("open_tab","incoming");putExtra("is_incoming_call",isIncoming);putExtra("phone_surface",true);addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_SINGLE_TOP or Intent.FLAG_ACTIVITY_REORDER_TO_FRONT)};applicationContext.startActivity(intent)}.onFailure{Log.e(TAG,"Unable to foreground CallShield in-call UI",it)}}
