...
object EmergencyRepeatCallPolicy {
    ...
    fun onTrustedIncomingCall(context: Context, number: String): Result {
        ...
        val threshold = configuredThreshold(prefs); val windowMinutes = configuredWindow(prefs); val now = System.currentTimeMillis(); val cutoff = now - windowMinutes * 60_000L; val normalized = normalize(number)
        if (normalized.isBlank()) return Result(false, 0, windowMinutes)
        val all = readEvents(prefs); val current = all.filter { it.number == normalized && it.timestamp >= cutoff }.toMutableList(); current += Event(normalized, now)
        writeEvents(prefs, current + all.filter { it.number != normalized && it.timestamp >= cutoff })
        val thresholdReached = current.size >= threshold; val lastEscalation = readEscalation(prefs, normalized); val cooldownElapsed = lastEscalation <= 0L || now - lastEscalation >= windowMinutes * 60_000L; val triggered = thresholdReached && cooldownElapsed
        if (triggered) { writeEscalation(prefs, normalized, now); escalate(context, number, current.size, windowMinutes) }
        return Result(triggered, current.size, windowMinutes)
    }
    private fun escalate(context: Context, number: String, count: Int, windowMinutes: Int) { val screenOff = context.getSystemService(PowerManager::class.java)?.isInteractive == false; val audio = context.getSystemService(AudioManager::class.java); if (screenOff && audio?.ringerMode == AudioManager.RINGER_MODE_SILENT) playShortEmergencyAlert(context); showEmergencyNotification(context, number, count, windowMinutes) }
    private fun playShortEmergencyAlert(context: Context) { runCatching { alertRingtone?.stop(); val uri = RingtoneManager.getActualDefaultRingtoneUri(context, RingtoneManager.TYPE_RINGTONE) ?: RingtoneManager.getDefaultUri(RingtoneManager.TYPE_RINGTONE); alertRingtone = RingtoneManager.getRingtone(context.applicationContext, uri); alertRingtone?.audioAttributes = AudioAttributes.Builder().setUsage(AudioAttributes.USAGE_ALARM).setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION).build(); alertRingtone?.play(); android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({ runCatching { alertRingtone?.stop() } }, 8_000L) } }
    private fun showEmergencyNotification(context: Context, number: String, count: Int, windowMinutes: Int) { if (!context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getBoolean("security_notifications", true)) return; createChannel(context); val privacy = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getBoolean("privacy_mode", true); val text = if (privacy) "A saved contact has called $count times in $windowMinutes minutes." else "$number: $count calls in $windowMinutes minutes."; val notification = NotificationCompat.Builder(context, CHANNEL_ID).setSmallIcon(android.R.drawable.ic_dialog_alert).setContentTitle("Repeated call needs your attention").setContentText(text).setStyle(NotificationCompat.BigTextStyle().bigText("This saved contact is calling repeatedly in a short period. Review the call before deciding whether it needs attention.")).setCategory(NotificationCompat.CATEGORY_CALL).setPriority(NotificationCompat.PRIORITY_MAX).setAutoCancel(true).setVisibility(if (privacy) NotificationCompat.VISIBILITY_PRIVATE else NotificationCompat.VISIBILITY_PUBLIC).build(); context.getSystemService(NotificationManager::class.java)?.notify(NOTIFICATION_ID, notification) }
    private fun createChannel(context: Context) { if (Build.VERSION.SDK_INT < 26) return; val manager = context.getSystemService(NotificationManager::class.java) ?: return; if (manager.getNotificationChannel(CHANNEL_ID) == null) manager.createNotificationChannel(NotificationChannel(CHANNEL_ID, "Repeated-call alerts", NotificationManager.IMPORTANCE_HIGH).apply { description = "Alerts when a saved contact calls repeatedly in a short period."; setSound(null, null); lockscreenVisibility = NotificationCompat.VISIBILITY_PRIVATE }) }
