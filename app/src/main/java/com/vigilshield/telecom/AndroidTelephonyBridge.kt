...
class AndroidTelephonyBridge(private val activity: Activity, private val webView: WebView) {
    ...
    init {
        NativeInCallService.bridge = this
        NativeInCallService.appContext = activity.applicationContext
        CallNotificationHelper.ensureChannel(activity.applicationContext)
    }
    ...
    @JavascriptInterface fun requestDefaultDialerRole(): Boolean {
        ...
        if (!manager.isRoleAvailable(RoleManager.ROLE_DIALER)) return false
        activity.startActivityForResult(manager.createRequestRoleIntent(RoleManager.ROLE_DIALER), 7001)
        return true
    }
    @JavascriptInterface fun requestCallScreeningRole(): Boolean {
        ...
        activity.startActivityForResult(manager.createRequestRoleIntent(RoleManager.ROLE_CALL_SCREENING), 7003)
        return true
    }
    ...
    @JavascriptInterface fun placeRealCall(number: String, accountHandleId: String?): String {
        ...
        return try {
            val extras = Bundle()
            val account = telecom.callCapablePhoneAccounts.firstOrNull { it.id == accountHandleId } ?: telecom.callCapablePhoneAccounts.firstOrNull()
    ...
    @JavascriptInterface fun sendDtmfTone(id: String, digit: String): Boolean {
        val call = NativeInCallService.activeCalls[id] ?: return false
        val tone = digit.firstOrNull() ?: return false
        call.playDtmfTone(tone); call.stopDtmfTone(); return true
    }
    ...
    private fun readContacts(limit: Int): JSONArray {
        ...
        activity.contentResolver.query(ContactsContract.CommonDataKinds.Phone.CONTENT_URI, projection, null, null, "${ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME} ASC")?.use { cursor ->
            var count = 0
            while (cursor.moveToNext() && count < limit.coerceIn(1, 2000)) {
    ...
    fun lookupName(number: String): String? {
        ...
        return activity.contentResolver.query(Uri.withAppendedPath(ContactsContract.PhoneLookup.CONTENT_FILTER_URI, Uri.encode(number)), arrayOf(ContactsContract.PhoneLookup.DISPLAY_NAME), null, null, null)?.use { cursor -> if (cursor.moveToFirst()) cursor.getString(0) else null }
    }

    private fun readCallLogs(limit: Int): JSONArray {
        val result = JSONArray()
        if (!hasCallLogPermission()) { NativeInCallService.readHistory(limit).forEach(result::put); return result }
    ...
    companion object { private const val PREFS = "vigilshield" }
}
...
class NativeInCallService : InCallService() {
    companion object {
        ...
        private fun startRinging() {
            stopRinging(); val context = appContext ?: return; val audio = context.getSystemService(AudioManager::class.java)
            when (audio.ringerMode) {
                ...
                AudioManager.RINGER_MODE_VIBRATE -> {
                    ...
                    ringtone?.audioAttributes = AudioAttributes.Builder().setUsage(AudioAttributes.USAGE_NOTIFICATION_RINGTONE).setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION).build()
                    ringtone?.play()
                }
            }
        }
        ...
        fun readHistory(limit: Int): List<JSONObject> {
            ...
            runCatching {
                val array = JSONArray(appContext?.getSharedPreferences("vigilshield", Context.MODE_PRIVATE)?.getString("call_history", "[]") ?: "[]")
                for (index in 0 until minOf(array.length(), limit.coerceIn(1, 500))) result += array.getJSONObject(index)
            }
            return result
        }

        fun swapCalls(): Boolean {
            val held = activeCalls.values.firstOrNull { it.state == Call.STATE_HOLDING }; val active = activeCalls.values.firstOrNull { it.state == Call.STATE_ACTIVE }
            if (held == null || active == null) return false; active.hold(); held.unhold(); return true
        }
        ...
        fun persist(call: Call) {
            val context = appContext ?: return; val prefs = context.getSharedPreferences("vigilshield", Context.MODE_PRIVATE); val array = JSONArray(prefs.getString("call_history", "[]") ?: "[]"); val id = ids[call] ?: return
            val number = call.details.handle?.schemeSpecificPart.orEmpty(); val name = bridge?.lookupName(number).orEmpty().ifBlank { call.details.callerDisplayName.orEmpty() }.ifBlank { number.ifBlank { "Unknown caller" } }
            var existing = -1; for (index in 0 until array.length()) if (array.optJSONObject(index)?.optString("id") == id) { existing = index; break }
            val connectedAt = call.details.connectTimeMillis; val duration = if (connectedAt > 0) ((System.currentTimeMillis() - connectedAt) / 1000).coerceAtLeast(0) else 0
            val type = if (call.details.callDirection == Call.Details.DIRECTION_INCOMING) "INCOMING" else "OUTGOING"
            val entry = JSONObject().put("id", id).put("number", number).put("callerName", name).put("type", type).put("timestamp", if (existing >= 0) array.getJSONObject(existing).optLong("timestamp", System.currentTimeMillis()) else System.currentTimeMillis()).put("durationSeconds", duration).put("isContact", bridge?.lookupName(number)?.isNotBlank() == true)
            if (existing >= 0) array.put(existing, entry) else array.put(0, entry)
            while (array.length() > 500) array.remove(array.length() - 1)
    ...
    override fun onCallAdded(call: Call) {
        ...
        val callback = object : Call.Callback() {
            override fun onStateChanged(c: Call, state: Int) {
                if (state == Call.STATE_RINGING && c.details.callDirection == Call.Details.DIRECTION_INCOMING) startRinging() else if (state != Call.STATE_RINGING) stopRinging()
                if (state == Call.STATE_DISCONNECTED) CallNotificationHelper.clearCall(applicationContext, id)
                emit(c, state); persist(c)
            }
        }
        callbacks[id] = callback; call.registerCallback(callback); persist(call)
        val incoming = call.details.callDirection == Call.Details.DIRECTION_INCOMING
        val number = call.details.handle?.schemeSpecificPart.orEmpty(); val name = bridge?.lookupName(number).orEmpty().ifBlank { call.details.callerDisplayName.orEmpty() }.ifBlank { number.ifBlank { "Unknown caller" } }
        if (incoming && call.state == Call.STATE_RINGING) { startRinging(); CallNotificationHelper.showIncomingCall(applicationContext, id, name, number) }
        emit(call, call.state)
    }

    private fun emit(call: Call, state: Int) {
        val id = ids[call] ?: return; val number = call.details.handle?.schemeSpecificPart.orEmpty(); val incoming = call.details.callDirection == Call.Details.DIRECTION_INCOMING
        val name = bridge?.lookupName(number).orEmpty().ifBlank { call.details.callerDisplayName.orEmpty() }
    ...
    override fun onCallRemoved(call: Call) {
        val id = ids[call] ?: return; val incoming = call.details.callDirection == Call.Details.DIRECTION_INCOMING; val number = call.details.handle?.schemeSpecificPart.orEmpty(); val name = bridge?.lookupName(number).orEmpty().ifBlank { call.details.callerDisplayName.orEmpty() }.ifBlank { number.ifBlank { "Unknown caller" } }
        if (incoming && call.details.connectTimeMillis <= 0L) CallNotificationHelper.showMissedCall(applicationContext, name, number)
        CallNotificationHelper.clearCall(applicationContext, id); persist(call); activeCalls.remove(id); callbacks.remove(id)?.let { call.unregisterCallback(it) }; ids.remove(call); stopRinging(); if (activeCalls.isEmpty()) instance = null
        super.onCallRemoved(call)
    }

    @Suppress("DEPRECATION")
    fun setSpeaker(enabled: Boolean): Boolean { setAudioRoute(if (enabled) CallAudioState.ROUTE_SPEAKER else CallAudioState.ROUTE_WIRED_OR_EARPIECE); return true }
}
