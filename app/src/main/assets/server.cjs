...
var dbPhoneNumbers = /* @__PURE__ */ new Map();
var dbPhoneIdByE164 = /* @__PURE__ */ new Map();
var dbCallerProfiles = /* @__PURE__ */ new Map();
var dbBusinessProfiles = /* @__PURE__ */ new Map();
var dbReputations = /* @__PURE__ */ new Map();
var dbReports = [];
var dbBlockRules = [];
function seedAuthorizedDirectory() {
  ...
  for (const s of seeds) {
    ...
    dbReputations.set(pId, {
      ...
      scam_score: s.scamScore || 0,
      fraud_score: s.riskScore > 80 ? 75 : 0,
      telemarketing_score: s.telemarketingScore || 0,
      robocall_score: s.riskScore > 70 ? 80 : 0,
      safe_score: s.safeScore,
      report_count: s.reportCount || 0,
      unique_reporter_count: s.reportCount ? Math.floor(s.reportCount * 0.85) : 0,
      recent_report_count: s.reportCount ? Math.floor(s.reportCount * 0.15) : 0,
      positive_feedback_count: s.safeScore > 80 ? 450 : 2,
      negative_feedback_count: s.reportCount || 0,
...
function normalizeInput(raw) {
  ...
  const parsed = (0, import_libphonenumber_js.parsePhoneNumberFromString)(cleaned, "US") || (0, import_libphonenumber_js.parsePhoneNumberFromString)(`+${cleaned.replace(/\D/g, "")}`);
...
app.get("/v1/reputation/:number", (req, res) => {
  ...
  return res.json({
    ...
    reputation: {
      ...
      last_calculated_at: (/* @__PURE__ */ new Date()).toISOString()
    }
  });
});
  ...
  if (pId) {
    ...
    if (profile || business) {
      return res.json({
        ...
        updatedAt: profile?.updated_at || business?.last_verified_at || (/* @__PURE__ */ new Date()).toISOString()
      });
    }
  }
  return res.json({
...
var reportRateMap = /* @__PURE__ */ new Map();
app.post("/v1/reports", (req, res) => {
  const ip = req.ip || "anonymous";
  const now = Date.now();
  const times = (reportRateMap.get(ip) || []).filter((t) => now - t < 6e4);
  if (times.length >= 10) {
  ...
  if (!pId) {
    ...
    dbReputations.set(pId, {
      ...
      last_reported_at: (/* @__PURE__ */ new Date()).toISOString(),
      last_calculated_at: (/* @__PURE__ */ new Date()).toISOString()
    });
  } else {
    const rep = dbReputations.get(pId);
    if (rep) {
      ...
      rep.last_reported_at = (/* @__PURE__ */ new Date()).toISOString();
      rep.last_calculated_at = (/* @__PURE__ */ new Date()).toISOString();
    }
  }
...
app.get("/v1/user/protection-stats", (req, res) => {
  res.json({
    ...
    lastSyncTimestamp: Date.now() - 1e3 * 60 * 30
  });
});
...
async function lookupWithIpqs(rawNumber) {
  const key = process.env.IPQS_API_KEY?.trim();
  if (!key) return null;
  const parsed = (0, import_libphonenumber_js.parsePhoneNumberFromString)(String(rawNumber || "").trim(), "US") || (0, import_libphonenumber_js.parsePhoneNumberFromString)(`+${String(rawNumber || "").replace(/\D/g, "")}`);
  const e164 = parsed?.isValid() ? parsed.format("E.164") : `+${String(rawNumber || "").replace(/\D/g, "")}`;
  ...
  try {
    ...
    return {
      ...
      name: data.name && data.name !== "N/A" ? String(data.name) : void 0,
      carrier: data.carrier && data.carrier !== "N/A" ? String(data.carrier) : void 0,
      lineType: data.line_type && data.line_type !== "N/A" ? String(data.line_type) : void 0,
      country: data.country && data.country !== "N/A" ? String(data.country) : void 0,
      region: data.region && data.region !== "N/A" ? String(data.region) : void 0,
      city: data.city && data.city !== "N/A" ? String(data.city) : void 0,
      valid: typeof data.valid === "boolean" ? data.valid : void 0,
...
app.post("/api/call-summary", async (req, res) => {
  try {
    ...
    if (!ai) {
      return res.json({
        summary: classification === "SPAM" || classification === "SCAM" ? `Likely unwanted solicitation or suspicious automated call from ${callerName || number}.` : classification === "VERIFIED" ? `Verified organization call from ${callerName || number}.` : `Standard voice call from ${callerName || number}.`,
        riskSignals: [
...
app.post("/api/screened-call-summary", async (req, res) => {
  ...
  const transcriptText = transcriptList.map((t) => `${t.sender === "caller" ? "Caller" : t.sender === "user" ? "User" : "AI Screener"}: "${t.text}"`).join("\n");
  try {
    const ai = getGeminiClient();
...
app.get("/vigilshield_app.zip", (req, res) => {
  const zipFile = import_path.default.join(process.cwd(), "public", "vigilshield_app.zip");
